...
##### 실험 개요
이번 실험에서는 문서 이미지 분류 문제를 해결하기 위해 다양한 딥러닝 모델과 최적화 기법이 적용되었습니다. 특히, 이미지의 변형과 클래스 간 불균형을 해결하기 위해 다양한 이미지 증강(Augmentation) 기법과 Oversampling 기법이 활용되었습니다. 또한, 최적의 백본 모델을 선정하고 하이퍼파라미터 튜닝을 통해 모델 성능을 극대화하고자 했습니다.

#### 주요 결과
1. **초기 데이터 전처리**에서는 오분류된 이미지를 재분류하고, 다양한 이미지 증강 기법을 통해 데이터를 증강했습니다. 이 과정에서 **Albumentation**과 **Augraphy** 라이브러리가 사용되었으며, Transpose, GaussianNoise, Blur 등의 기법이 적용되었습니다. 이로 인해 학습 데이터의 다양성이 확보되었고, 모델의 일반화 성능이 향상되었습니다.

2. **EfficientNet B5 기반 모델**의 활용에서, 문서의 가로 세로 비율을 유지하면서 이미지를 패딩하는 전처리 방법이 성능 향상에 기여했습니다. 특히, 특정 클래스(3, 7, 14)에 대해 성능이 떨어지는 것을 확인하고, 해당 클래스에 대한 가중치 조정 및 별도 학습을 통해 성능을 개선했습니다. 결과적으로, 최종 리더보드에서 0.9631의 높은 F1 점수를 달성했습니다.

3. **하이브리드 모델 구성**에서는 EfficientNet과 Swin Transformer의 장점을 결합한 Hybrid EfficientNet Swin-Transformer 모델이 사용되었습니다. 이 모델은 ImageNet으로 사전학습된 EfficientNet 인코더와 Swin Transformer 블록을 결합하여 높은 성능을 기록했습니다. 이 모델을 K-Fold Cross-Validation을 통해 학습한 결과, Validation F1 점수가 0.8224로 개선되었습니다.

4. **Test-Time Augmentation(TTA)** 기법이 Inference 단계에서 적용되었습니다. 평가 이미지에 Flip, RandomRotate 등의 변형을 가해 여러 번의 예측을 수행한 후, Soft-Voting 앙상블을 통해 최종 결과를 도출했습니다. 이를 통해 최종 리더보드에서 0.9397의 F1 점수를 달성했습니다.

5. **앙상블 기법**은 다양한 모델의 예측 결과를 종합하여 성능을 극대화하는 데 사용되었습니다. 특히, Hard-Voting 앙상블을 통해 각 모델의 강점을 결합하여 최종 리더보드에서 최고 성능을 달성하는 데 기여했습니다.

#### 결론
이번 실험에서는 다양한 이미지 증강 기법과 최적화 기법을 통해 문서 이미지 분류 성능을 크게 향상시킬 수 있었습니다. 특히, 특정 클래스에 대한 별도 학습과 Test-Time Augmentation을 통한 예측 강화가 성능 향상에 중요한 역할을 했습니다. 마지막으로, 여러 모델을 결합한 앙상블 기법을 통해 최종적으로 높은 성능을 달성한 것이 주목할 만한 성과입니다.
